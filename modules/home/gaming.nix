...
in
  outputs.lib.mkModule config false "gaming"
  {
  ...
  (let
    ...
    isGamingEnabled =
      config.modules.gaming.wine.enable
      || config.modules.gaming.proton.enable
      || config.modules.gaming.steam.enable
      || config.modules.gaming.lutris.enable
      || config.modules.gaming.games.minecraft.enable
      || config.modules.gaming.games.pokemmo.enable;
  in (outputs.lib.mkFor system hostname {
    systems.linux = {
      home = {
        packages = let
          # TODO: awful formatting
          winePkgs = outputs.lib.optionals config.modules.gaming.wine.enable (
            (with pkgs; [bottles])
          ...
          protonPkgs = outputs.lib.optionals config.modules.gaming.proton.enable (
            with pkgs; [protonup]
          );
          lutrisPkgs = outputs.lib.optionals config.modules.gaming.lutris.enable (
            with pkgs; [lutris]
          );
          minecraftPkgs = outputs.lib.optionals config.modules.gaming.games.minecraft.enable (
            with pkgs; [prismlauncher]
          );
          pokemmoPkgs = outputs.lib.optionals config.modules.gaming.games.pokemmo.enable (
        ...
        activation = {
          # Run protonup once
          protonup = outputs.lib.mkIf config.modules.gaming.proton.enable (
      ...
      nixos = {
        ...
        programs = {
          ...
          steam = outputs.lib.mkIf config.modules.gaming.steam.enable {
            enable = true;
